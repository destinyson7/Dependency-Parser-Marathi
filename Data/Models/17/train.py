...
N = len(words) + len(chunk_tags) + len(morph_vectors) + \
    len(words) + len(chunk_tags) + len(morph_vectors) + 2
    ...
    for line in f:
        if line.strip():
            ...
            if current[1].strip().split(" ")[0].strip() == "ROOT":
                ...
                col_ind.append(words_len + chunk_len + morph_len +
                               words_len + chunk_tags.index("ROOT"))
                col_ind.append(words_len + chunk_len + morph_len +
                               words_len + chunk_len + morph_index["ROOT"])
                ...
                col_ind.append(words_len + chunk_len + words_len + morph_len +
                               chunk_index[(current[1].strip().split(" ")[3])])
                col_ind.append(words_len + chunk_len + words_len + morph_len +
                               chunk_len + morph_index[" ".join(current[1].strip().split(" ")[7:]).strip()])

            col_ind.append(2 * (words_len + chunk_len + morph_len) +

...
N = len(words) + len(pos_tags) + len(morph_vectors) + \
    len(words) + len(pos_tags) + len(morph_vectors)
    ...
    for line in f:
        if line.strip():
            ...
            if current[0].strip().split(" ")[0].strip() == "ROOT":
                ...
                col_ind.append(words_len + pos_index["ROOT"])
                col_ind.append(words_len +
                               pos_len + morph_index["ROOT"])
                ...
                col_ind.append(words_len +
                               pos_index[(current[0].strip().split(" ")[2])])
                col_ind.append(words_len + pos_len +
                               morph_index[" ".join(current[0].strip().split(" ")[7:]).strip()])

            if current[1].strip().split(" ")[0].strip() == "ROOT":
                col_ind.append(words_len +
                               pos_len + morph_len + word_index[("ROOT")])
                # col_ind.append(words_len + pos_len + pos_len +
                #                words_len + morph_len + pos_tags.index("ROOT"))
                col_ind.append(words_len + pos_len + morph_len +
                               words_len + pos_tags.index("ROOT"))
                col_ind.append(words_len + pos_len + morph_len +
                               words_len + pos_len + morph_index["ROOT"])

            else:
                # print(current[1].strip().split(" ")[1])
                col_ind.append(words_len + pos_len + morph_len +
                               word_index[(current[1].strip().split(" ")[1])])
                # col_ind.append(words_len + pos_len + pos_len + words_len + morph_len +
                #                pos_index[(current[1].strip().split(" ")[2])])
                col_ind.append(words_len + pos_len + words_len + morph_len +
                               pos_index[(current[1].strip().split(" ")[2])])
                col_ind.append(words_len + pos_len + words_len + morph_len + pos_len +
                               morph_index[" ".join(current[1].strip().split(" ")[7:]).strip()])

            row_ind.extend(repeat(len(Y), 6))
